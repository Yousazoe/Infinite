...
void IcarusLevel::OnUpdate(Infinite::Timestep ts)
{
...
void IcarusLevel::OnImGuiRender()
{
	m_Player.OnImGuiRender();
}

void IcarusLevel::CreatePillar(int index, float offset)
{
	...
	float center = IcarusRandom::Float() * 35.0f - 17.5f;
	float gap = 2.0f + IcarusRandom::Float() * 5.0f;

	pillar.TopPosition.y = 10.0f - ((10.0f - center) * 0.2f) + gap * 0.5f;
	pillar.BottomPosition.y = -10.0f - ((-10.0f - center) * 0.2f) - gap * 0.5f;
}

bool IcarusLevel::CollisionTest()
{
...
void IcarusLevel::GameOver()
{
	m_GameOver = true;
}

void IcarusLevel::Reset()
{
	m_GameOver = false;
